...
const SIPRegistrationReceipt = () => {
  ...
  const {
    data: sipReceiptsData,
    isSuccess: sipReceiptsSuccess,
  ...
  const { data: sipReceiptsDataByBoid } = useGetSipReceiptsByBoid(boid);

  const totalPage = Math.ceil(sipReceiptsData?.meta?.records / 10);

  useEffect(() => {
    sipReceiptDataRefetch();
    if (sipReceiptsData?.responseData?.next === null) {
      setNextPage(true);
    } else {
      setNextPage(false);
    }
    if (sipReceiptsData?.responseData?.previous === null) {
      setPrevPage(true);
    } else {
      setPrevPage(false);
    }
  }, [sipReceiptsData, sipReceiptsSuccess, sipReceiptDataRefetch]);

  useEffect(() => {
    if (sipReceiptsSuccess) {
      setTableData(sipReceiptsData?.responseData?.results);
    }
  }, [sipReceiptsData, sipReceiptsSuccess]);

  const handleLoad = () => {
    if (boid) {
      setFilteredTableData(sipReceiptsDataByBoid?.responseData?.results || []);
