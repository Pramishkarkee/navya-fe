...
const UnitPurchaseChart = ({ data }) => {
  ...
  const transformedData = data?.responseData?.data
    ?.map(
      (item: {
        ...
        unit_purchase: number | string | any;
        unit_purchase_amount: number | string | any;
      }) => {
        const date =
          item.day instanceof Date
            ? item.day.getTime()
            : new Date(item.day).getTime();

        return {
          x: date,
          y: parseFloat(item.unit_purchase_amount ?? item.unit_purchase ?? 0),
        };
      }
    )
  ...
  const options = {
    ...
    tooltip: {
      enabled: true,
      pointFormat: "Unit Purchase: <b>{point.y:.1fr}</b>",
      positioner: function (
        labelWidth: number,
        labelHeight: number,
    ...
    series: [
      {
        ...
        tooltip: {
          pointFormat: `Unit Purchase: {point.y}`,
        },
      },
    ],
  };
...
export default UnitPurchaseChart;
