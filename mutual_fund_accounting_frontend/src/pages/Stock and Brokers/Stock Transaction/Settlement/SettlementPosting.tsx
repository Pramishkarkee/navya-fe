...
import {
  useGetAllPendingStockSettlement,
  usePostApproveStockSettlement,
  usePostRejectStockSettlement,
...
const SettlementPosting = () => {
  ...
  const { data: pendingStockSettlmentList } = useGetAllPendingStockSettlement(
    searchData.from_date,
    searchData.to_date,
    searchValue,
    pagination.pageIndex + 1
  );
  const { mutate: ApproveStockSettlement, isPending: PendingApprovedStock } =
    usePostApproveStockSettlement();
  const { mutate: RejectStockSettlement } = usePostRejectStockSettlement();

  const totalPageCount = Math.ceil(pendingStockSettlmentList?.meta?.count / 10);

  useEffect(() => {
    if (pendingStockSettlmentList?.meta?.next === null) {
      setNext(true);
    } else {
      setNext(false);
    }
    if (pendingStockSettlmentList?.meta?.previous === null) {
      setPrev(true);
    } else {
      setPrev(false);
    }
  }, [pendingStockSettlmentList]);
  ...
  const handleApproveTransaction = (row: any) => {
    ...
    ApproveStockSettlement(payload, {
      onSuccess: () => {
  ...
  useEffect(() => {
    if (pendingStockSettlmentList?.responseData) {
      setDisplayData(pendingStockSettlmentList?.responseData);
    } else {
      setDisplayData([]);
    }
  }, [pendingStockSettlmentList]);
    ...
    <React.Fragment>
      ...
      >
        <Box sx={{ width: "50px" }}>
          ...
          >
            List of Pending Stock Settlement
          </Typography>
        </Box>
        <Box
          component="form"
        ...
        >
          {pendingStockSettlmentList?.responseData?.length > 0 ? (
            <TableComponent
              data={pendingStockSettlmentList?.responseData}
              columns={columns}
...
export default SettlementPosting;
