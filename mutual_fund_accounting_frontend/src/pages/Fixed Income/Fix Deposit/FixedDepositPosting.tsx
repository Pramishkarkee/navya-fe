...
import {
  useGetPendingFixedDepositList,
  usePostApproveFixedDepositPosting,
  usePostRejectFixedDepositPosting,
} from "services/Fixed Deposit/FixedDepositService";
import { FDTableList } from "constants/FixedDepositTable/FixedDepositTableHeaders";
...
interface FixedDeposit extends FDTableList {
  subRows?: FDTableList[];
}

const FixedDepositPosting = () => {
  ...
  const [displayData, setDisplayData] = useState<FixedDeposit[]>([]);
  ...
  const { data: pendingFixedDepositData } = useGetPendingFixedDepositList(
  ...
  const handleApproveTransaction = (row: any) => {
    ...
    ApprovePendingFixedDeposit(payload, {
      onSuccess: () => {
        setSuccessMessage("Fixed Income Approved Successfully.");
  ...
  const handleRejectTransaction = (row: any) => {
    ...
    RejectPendingFixedDeposit(payload, {
      onSuccess: () => {
        setSuccessMessage("Fixed Income Rejected Successfully.");
        setSnackbarOpen(true);
        setSelectedTransactions([]);
      },
      onError: () => {
  ...
  const handleRowSelect = (row: FixedDeposit) => {
    ...
    <Box display="flex" alignItems="center">
      <Box sx={{ ml: 3 }}>
        ...
        <Typography variant="body1" sx={{ mt: 1 }}>
          {row.original.interest_interval}
        </Typography>
      </Box>
      ...
      <Box sx={{ ml: 3 }}>
        ...
        <Typography variant="body1" sx={{ mt: 1 }}>
          {row.original.cancellation_charge}
        </Typography>
      </Box>
...
export default FixedDepositPosting;
